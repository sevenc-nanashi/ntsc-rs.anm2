...
desc "Luaスクリプトをビルドして./lua/ntsc-rs_old.anm2に出力します"
task :build_old_script do
  require "yaml"
  require "json"
  require "tomlrb"

  source = File.read("./lua/old_script.lua")
  ...
  File.write("./lua/ntsc-rs_old.anm2", output)
  ...
  puts "Build completed: ./lua/ntsc-rs_old.anm2"
end
...
task :build_script do
  ...
  flat_parameters.each do |param_key, param_value|
    ...
    label = current_group ? "#{current_group}::#{param_value["label"]}" : param_value["label"]
    if param_value["type"] == "group"
      current_group = param_value["label"]
      definition = "--group:#{param_value["label"]},#{param_value["opened"]}"
    else
      definition = "--#{param_value["type"]}@PARAM_#{param_key}:#{label}"
      case param_value["type"]
      when "group"
...
task :build_aul2 do
  ...
  File.open("./target/English.ntsc-rs.aul2", "w") do |file|
    ...
    これは旧バージョンからの移行用スクリプトです。新しいスクリプトを使用してください。=This is a migration script from the old version. Please use the new script.
